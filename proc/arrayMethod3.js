...
const madId = users.find((user) => user.firstName == "Madison" && user.lastName == "Marshall")
console.log(madId)

// 2) Who wrote the first comment (assuming the first comment is in position 0 of the comments array)
const whoWroteFirstComment = comments[0];

const nameOfwroteThePerson = users.find((user) => user.id == whoWroteFirstComment.userId);
console.log(nameOfwroteThePerson)


// 3) Which user commented 'OK great thanks'?
const userOk = comments.find((comment) => comment.text === "OK great thanks")
const userNameWithOk = users.find((user) => user.id == userOk.userId);
console.log([userOk, userNameWithOk])


// 4) Add the user's first and last name to each comment in the comments array
// const addNameToComment =
